...
plrd <- function (Y, X, threshold,
  ...
  n = length(Y)
  W = (X>=threshold)
  df = data.frame(Y = Y, Xc = X - threshold, W = W)
  fit1 = stats::lm(Y ~ W * I(Xc) + I(Xc^2) + I(Xc^3), 
                   data = df, subset = abs(Xc) < max.window)
  fit2 = stats::lm(Y ~ W * (I(Xc) + I(Xc^2) + I(Xc^3)),
                   data = df, subset = abs(Xc) < max.window)
  # diff.curvatures = as.numeric(lmtest::waldtest(fit1, fit2,
  #                                                 vcov = vcovHC(fit2, type = "HC3"),
  #                                                 test = "F")[["Pr(>F)"]][2]) <= signif.curvature
